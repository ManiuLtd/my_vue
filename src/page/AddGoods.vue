  ...
  <div>
    ...
    <div class="add_header">
      ...
      <router-link to="/brand">
        ...
        <span>{{brandInfo}} <router-link to="/brand"><img src="../assets/images/icon_arrow_right.png"/></router-link></span>
      </div>
      </router-link>
      <!--商品介绍-->
      ...
      <div class="add_content_bottom">
        <textarea placeholder="输入该商品的详细描述(30字以内)" maxlength="30" v-model.trim="introduction"></textarea>
      </div>
      <!--商品头图-->
      ...
      <div class="add_box">
        ...
        <span>
          <radio-button radio_name="active" radio_title="参与" style="margin-right: 1rem;" is-checked="true" radio_val="1" @radioChange="getisJionActivity"></radio-button>
          <radio-button radio_name="active" radio_title="不参与" radio_val="2" @radioChange="getisJionActivity"></radio-button>
        </span>
      </div>
      <!--限购数量-->
      <div class="add_box">
      ...
      <div class="add_box" style="border: none;">
        ...
        <span>
          <radio-button radio_name="g_status" radio_title="审核通过后，立即自动上架" is-checked="true" radio_val="4" @radioChange="getProductState"></radio-button>
        </span>
      </div>
      <div class="add_box">
        ...
        <span>
          <radio-button radio_name="g_status" radio_title="审核通过后，手动操作上架" radio_val="1" @radioChange="getProductState"></radio-button>
        </span>
      </div>
    </div>
    <!--协议部分-->
      ...
      <p class="agreement">
        <van-checkbox v-model.trim="protocolChecked">
          ...
          <router-link to="/goodsRelease">
            <span style="color: #4cc3ad;font-size: .4rem;">《商品发布规格》</span>
          </router-link>
        </van-checkbox>
      </p>
    </div>
    <span class="preserve_btn" @click="submit">确定添加</span>
  </div>
</template>
  ...
  export default {
    ...
    methods:{
      submit(){
        ...
        (new Promise(function(resolve){
          ...
          that.$post(requestUrl,data).then((response)=>{
            ...
            that.$router.replace("/addGoodSuccess");
            loading.close();
          }).then((error)=>{
            loading.close();
          });
      ...
      getAttributeData(id){
        let loading = new Loading();
        loading.show();
        this.$get(API.ATTRIBUTE_DATA + `?gc_id=${id}`).then((response)=>{
          if(response.code != 200){
            new Toast(response.msg).show();
            return;
          }
      ...
      uploadImg(){
        ...
        this.postLogoImage(this.productLogoExt[0]);
      }
    },
